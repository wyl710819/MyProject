...
public class BadgeRadarView extends View {
	...
	private int count;
    private int[] radius = new int[5];
    private int maxRadius = radius[radius.length - 1];
    private int[] marks;
    private String[] keys;
    private String[] grade= new String[]{"10", "20", "30", "40", "50"};
	...
	public BadgeRadarView(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public BadgeRadarView(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	public void initRadius(Context context){
		for(int i=0;i<radius.length;i++){
			radius[i] = dip2px(getContext(), 28*(i+1));
		}
...
//	            keys[k] = key;
	        ...
	        for (int i = 0; i < maxCountText.length; i++) {
	        	maxTexts[i]=maxCountText[i]>maxCountTexts[i] ? maxCountText[i] : maxCountTexts[i];
			}
...
//	        paintMarkLine.setAlpha(100);
	        ...
	        Path path = new Path();
	        path.reset();
	        double xTag = getPointX(littleAngle * 0, ratioS[0]<1 ? marks[0]*ratioS[0]  : marks[0]*(0==maxCountIndex ? 1 : ratioS[0]))+5;
	        double yTag = getPointY(littleAngle * 0, ratioS[0]<1 ? marks[0]*ratioS[0]  : marks[0]*(0==maxCountIndex ? 1 : ratioS[0]));
	        for (int i = 0; i < count; i++) {
	        	 
	 		   if (propTags.contains(i)) {
	 			   
	 			  if (marks[i]==0) {
	 				    x = getPointX(littleAngle * i, maxCount*(maxRadius/Double.valueOf(grade[grade.length-1])))+widthTag;
			            y = getPointY(littleAngle * i, maxCount*(maxRadius/Double.valueOf(grade[grade.length-1])))+widthTag;
					}else {
						x = getPointX(littleAngle * i, maxCount*(maxRadius/Double.valueOf(grade[grade.length-1]))-(marks[i]*ratioS[i]))+widthTag;
			            y = getPointY(littleAngle * i, maxCount*(maxRadius/Double.valueOf(grade[grade.length-1]))-(marks[i]*ratioS[i]))+widthTag;
					}
	 			  
	 			}else {
	 				if (ratioS[i]<1) {
	 					 x = getPointX(littleAngle * i, marks[i]*ratioS[i])+widthTag;
	 		            y = getPointY(littleAngle * i, marks[i]*ratioS[i])+widthTag;
					}else {
						x = getPointX(littleAngle * i, marks[i]*(i==maxCountIndex ? 1 : ratioS[i]))+widthTag;
			            y = getPointY(littleAngle * i, marks[i]*(i==maxCountIndex ? 1 : ratioS[i]))+widthTag;
					}
...
//	        paintMarkLine2.setAlpha(100);
	        ...
	        Path path2 = new Path();
	        path2.reset();
//	        double xTag2 = getPointX(littleAngle * 0, markS[0]*(0==maxCountIndex ? 1 : ratioS[0]))+5;
//	        double yTag2 = getPointY(littleAngle * 0, markS[0]*(0==maxCountIndex ? 1 : ratioS[0]));
	        double xTag2 = getPointX(littleAngle * 0, ratioS[0]<1 ? markS[0]*ratioS[0]  : markS[0]*(0==maxCountIndex ? 1 : ratioS[0]))+5;
	        double yTag2 = getPointY(littleAngle * 0, ratioS[0]<1 ? markS[0]*ratioS[0]  : markS[0]*(0==maxCountIndex ? 1 : ratioS[0]));
	        for (int i = 0; i < count; i++) {
	        	 if (propTags.contains(i)) {
	        		 
	        		 if (markS[i]==0) {
	        			 x2 = getPointX(littleAngle * i, maxCount*(maxRadius/Double.valueOf(grade[grade.length-1])))+widthTag;
	 	 	            y2 = getPointY(littleAngle * i, maxCount*(maxRadius/Double.valueOf(grade[grade.length-1])))+widthTag;
					}else {
						x2 = getPointX(littleAngle * i, maxCount*(maxRadius/Double.valueOf(grade[grade.length-1]))-(markS[i]*ratioS[i]))+widthTag;
			            y2 = getPointY(littleAngle * i, maxCount*(maxRadius/Double.valueOf(grade[grade.length-1]))-(markS[i]*ratioS[i]))+widthTag;
		 	          
					}
		 			}else {
		 				
		 				if (ratioS[i]<1) {
		 					 x2 = getPointX(littleAngle * i, markS[i]*ratioS[i])+widthTag;
		 		            y2 = getPointY(littleAngle * i, markS[i]*ratioS[i])+widthTag;
						}else {
							x2 = getPointX(littleAngle * i, markS[i]*(i==maxCountIndex ? 1 : ratioS[i]))+widthTag;
				            y2 = getPointY(littleAngle * i, markS[i]*(i==maxCountIndex ? 1 : ratioS[i]))+widthTag;
						}
		 				
					}
//	        	 Log.e("run", "x2:"+x2+"    ,     "+"y2:"+y2);
	   ...
	   private double getRatio(Integer value, int j) {
		   double a=((value/5)+1);
		   maxCountText[j]=(int) (a*5);
		return maxCount/(a*5);
	}
	   
	   private double getRatios(Integer value, int k) {
		   double a=((value/5)+1);
		   maxCountTexts[k]=(int) (a*5);
		  
		return maxCount/(a*5) > ratios[k] ? ratios[k] : maxCount/(a*5);
	}
...
//					}
	            ...
	            if (radius == maxRadius) {
					...
					case 1:
						...
						canvas.drawText(keys[i], (float) (x +setLength*Math.sin(angle)-mRect.width()/2), 
								(float) (y - setLength*Math.cos(angle)+mRect.height()/2-dip2px(getContext(),15)), paintText);
						
						canvas.drawText(String.valueOf(maxTexts[i]), (float) (x+dip2px(getContext(),25) +setLength*Math.sin(angle)-mRect.width()/2), 
								(float) (y - setLength*Math.cos(angle)+mRect.height()/2+dip2px(getContext(),4)), paintText);
					...
					case 2:
						...
						canvas.drawText(keys[i], (float) (x +setLength*Math.sin(angle2)-mRect.width()/2), 
								(float) (y + setLength*Math.cos(angle2)+mRect.height()/2+dip2px(getContext(),15)), paintText);
						
						canvas.drawText(String.valueOf(maxTexts[i]), (float) (x +dip2px(getContext(),25)+setLength*Math.sin(angle2)-mRect.width()/2), 
								(float) (y + setLength*Math.cos(angle2)+mRect.height()/2-dip2px(getContext(),4)), paintText);
						break;
					case 3:
						if (isBadge) {
							x+=dip2px(getContext(),10);
						...
						canvas.drawText(keys[i], (float) (x -setLength*Math.sin(angle3)-mRect.width()/2), 
								(float) (y + setLength*Math.cos(angle3)+mRect.height()/2+dip2px(getContext(),15)), paintText);
						
						canvas.drawText(String.valueOf(maxTexts[i]), (float) (x +dip2px(getContext(),25)-setLength*Math.sin(angle3)-mRect.width()/2), 
								(float) (y + setLength*Math.cos(angle3)+mRect.height()/2-dip2px(getContext(),4)), paintText);
						break;
					case 4:
						double angle4 = 2*Math.PI/360*(360-littleAngle*i);
						canvas.drawText(keys[i], (float) (x -setLength*Math.sin(angle4)-mRect.width()/2), 
								(float) (y - setLength*Math.cos(angle4)+mRect.height()/2-dip2px(getContext(),15)), paintText);
						canvas.drawText(String.valueOf(maxTexts[i]), (float) (x+dip2px(getContext(),25) -setLength*Math.sin(angle4)-mRect.width()/2), 
								(float) (y - setLength*Math.cos(angle4)+mRect.height()/2+dip2px(getContext(),4)), paintText);
						break;
		...
		private ArrayList<Integer> propTags=new ArrayList<Integer>();
	    /**
	     * 得到需要计算的角度
	     */
	    ...
	    public void setStudentMap(LinkedHashMap<String, Integer> studentMap) {
	        ...
	        ratios = new double[count];
	        maxCountText = new int[count];
			maxCountTexts = new int[count];
			maxTexts = new int[count];
	    }
...
//	        lastYs=new double[grade.length-1];
		...
		public void setMaxCount(int maxCount, int maxCountIndex, ArrayList<Integer> propTags) {
			int a=((maxCount/5)+1);
			this.maxCount =a*5;
			this.maxCountIndex=maxCountIndex;
			this.grade = new String[]{a+"", a*2+"", a*3+"", a*4+"", a*5+""};
