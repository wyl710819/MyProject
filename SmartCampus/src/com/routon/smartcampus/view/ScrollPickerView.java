...
public abstract class ScrollPickerView<T> extends View {
    ...
    private OnChangedListener mChanged;
     ...
     */
    public abstract void drawItem(Canvas canvas, List<String> data, int position, int relative, float moveLength, float top);
    ...
    private void fling(float from, float vel) {
        if (mIsHorizontal) {
            ...
            mIsFling = true;
            // 最多可以惯性滑动10个item
            mScroller.fling((int) from, 0, (int) vel, 0, -6 * mItemWidth,
                    6 * mItemWidth, 0, 0);
        } else {
            mLastScrollY = (int) from;
            mIsFling = true;
            // 最多可以惯性滑动10个item
            mScroller.fling(0, (int) from, 0, (int) vel, 0, 0, -6 * mItemHeight,
                    6 * mItemHeight);
        }
        invalidate();
    }
        ...
        if (end != 0) { // itemHeight为0导致endy=0
            mAutoScrollAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    computeScroll((Integer)animation.getAnimatedValue(), end, rate);
                }
            });
    ...
    public void autoScrollTo(final int endY, long duration, final Interpolator interpolator, boolean canIntercept) {
        ...
        mAutoScrollAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                float rate = 0;
                rate = animation.getCurrentPlayTime() * 1f / animation.getDuration();
                computeScroll((Integer) animation.getAnimatedValue(), endY, rate);
            }
        });
        mAutoScrollAnimator.removeAllListeners();
        mAutoScrollAnimator.addListener(new AnimatorListenerAdapter() {
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            ...
            if (mCanTap && !isScrolling() && !mIsScrollingLastTime) {
                if (lastMove >= mCenterPoint && lastMove <= mCenterPoint + mItemSize) {
                    ...
                    int move = mItemSize;
                    autoScrollTo(move, 100, sAutoScrollInterpolator, false);
