...
import static com.routon.smartcampus.graffiti.DrawUtil.GRAFFITI_PIXEL_UNIT;
import static com.routon.smartcampus.graffiti.DrawUtil.computeAngle;
import static com.routon.smartcampus.graffiti.DrawUtil.drawArrow;
import static com.routon.smartcampus.graffiti.DrawUtil.drawCircle;
import static com.routon.smartcampus.graffiti.DrawUtil.drawLine;
import static com.routon.smartcampus.graffiti.DrawUtil.drawRect;
...
public class GraffitiView extends View {
    ...
    // 保存涂鸦操作，便于撤销
    public CopyOnWriteArrayList<Undoable> mUndoStack = new CopyOnWriteArrayList<Undoable>();
    ...
    private CopyOnWriteArrayList<Undoable> undoStack = new CopyOnWriteArrayList<Undoable>();
    private CopyOnWriteArrayList<GraffitiPath> pathStack = new CopyOnWriteArrayList<GraffitiPath>();
    private CopyOnWriteArrayList<GraffitiSelectableItem> selectableStack = new CopyOnWriteArrayList<>();
    ...
    public GraffitiView(Context context, Bitmap bitmap, String eraser, boolean eraserImageIsResizeable, GraffitiListener listener) {
        ...
        if (eraser != null) {
            mBitmapEraser = createBitmapFromPath(eraser, getContext());
        }
    ...
    public  Bitmap createBitmapFromPath(String path, Context context) {
        WindowManager manager = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
        Display display = manager.getDefaultDisplay();
        int screenW = display.getWidth();
        int screenH = display.getHeight();
        return createBitmapFromPaths(path, screenW, screenH);
    }
    private Bitmap createBitmapFromPaths(String path, int maxWidth, int maxHeight) {
        Bitmap bitmap = null;
    ...
    private int computeBitmapSimple(int realPixels, int maxPixels) {
        if (maxPixels <= 0) {
            return 1;
        }
        try {
...
//            }
    ...
    public final void addSelectableItem(GraffitiSelectableItem item) {
        ...
        selectableStack.add(item);
        undoStack.add((Undoable)item);
    }
     ...
     */
    public void udo() {
        if (undoStack.size() > mUndoStack.size()) {
        	Undoable undoable =undoStack.get(mUndoStack.size());
    ...
    public float getSelectedItemSize() {
        ...
        Log.e("mSelectedItem", "=="+mSelectedItem.getSize());
